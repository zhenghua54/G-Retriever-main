...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #ajejc 在初始化中添加自注意力层
        # 这里使用模型的嵌入层来获得嵌入的维度
        # embedding_dim = self.word_embedding.embedding_dim  # 获取嵌入维度

        # # 在初始化时定义自注意力层
        # self.attention_layer = nn.MultiheadAttention(embed_dim=embedding_dim, num_heads=8).to(self.model.device)
