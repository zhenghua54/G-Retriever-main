...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            batch['graph'].x = batch['graph'].x.to(device)
            batch['graph'].edge_attr = batch['graph'].edge_attr.to(device)
            batch['graph'].edge_index = batch['graph'].edge_index.to(device)
        ...
        with torch.no_grad():
            for step, batch in enumerate(val_loader):
                try:
                    ...
                    print(f"Step {step} encountered an IndexError: {e}. Skipping this step.")
                    continue  # 略过当前 step，继续下一个
